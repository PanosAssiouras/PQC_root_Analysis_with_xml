...
void Meander_PQC_Flute3_Analysis_Final(int nFiles)
{
 ...
 //   gROOT->Reset();
   ...
   //TString Data_Dir = "/home/akyriakis/Diode-Measurements_Analysis/PQC_Measurements/Data_New_Format/";
   //TString Meander_DataFileName[600];
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<2; j++){
         ...
         ofstream xml_file;
         //        xml_file.open(Data_Dir+"/"+Diode_DataFileName[4*i+j](0,pos1)+"/"+xml_filename[4*i+j]);
         xml_file.open("./XML_Info_Flute3/" +xml_filename);
        ...
        Meander_xml.SaveDoc(Meander_xmldoc, "./XML_Info_Flute3/" +xml_filename);
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        // Save document to file
        CloverMetal_xml.SaveDoc(CloverMetal_xmldoc, "./XML_Info_Flute3/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        // Save document to file
        VDPEdge_xml.SaveDoc(VDPEdge_xmldoc, "./XML_Info_Flute3/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        LineWidthEdge_xml.SaveDoc(LineWidthEdge_xmldoc, "./XML_Info_Flute3/" +xml_filename);
        // Release memory before exit
...
void VDPBulk_PQC_Flute3_Analysis_Final(int nFiles)
{
 ...
 //   gROOT->Reset();
   ...
   //TString Data_Dir = "/home/akyriakis/Diode-Measurements_Analysis/PQC_Measurements/Data_New_Format/";
   //.qTString VDPBulk_DataFileName[600];
   for(int i=0;i<nFiles;i++) {
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        // Save document to file
        VDPBulk_xml.SaveDoc(VDPBulk_xmldoc, "./XML_Info_Flute3/" +xml_filename);
        // Release memory before exit
...
void Diode_IV_PQC_xml_production(int xml_on, int nFiles){
 ...
 //        cout << Diode_DataFileName[2*i+j](0,pos1) << endl;
         cout << xml_filename << endl;
         ofstream xml_file;
 //        xml_file.open(Data_Dir+"/"+Diode_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
         xml_file.open("./XML_Info_Flute3/" +xml_filename);
         //xml_filename.open(Form("",HM_Id));
        ...
        // Save document to file
        Diode_IV_xml.SaveDoc(Diode_IV_xmldoc, "./XML_Info_Flute3/" +xml_filename);
        // Release memory before exit
...
//        cout << Diode_CV_DataFileName[2*i+j](0,pos1) << endl;
        cout << xml_filename << endl;
        ofstream xml_file;
//        xml_file.open(Data_Dir+"/"+Diode_CV_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
        xml_file.open("./XML_Info_Flute3/"+xml_filename);
        //xml_filename.open(Form("",HM_Id));
        // First create engine
...
//              Diode_CV_xml.NewChild(DATA, 0, "FILE_NAME", xml_filename[2*i+j]);
        ...
        // Save document to file
//        Diode_CV_xml.SaveDoc(Diode_CV_xmldoc, Data_Dir+"/"+Diode_CV_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
        Diode_CV_xml.SaveDoc(Diode_CV_xmldoc, "./XML_Info_Flute3/"+xml_filename);
        // Release memory before exit
...
void Flute3_2_S_Extended_Characterization_with_xml(int nFiles)
{
   ...
   CVS_Output_File.open(Form("CSV_Info/Flute3_VPX%d_0xx_2-S_HM_E_W_Left_Extended_Info.csv",HM_Id));
