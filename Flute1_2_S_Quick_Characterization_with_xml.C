...
string Structure_Id[40] = {"03","15","27","37"}; //wafers to be analyzed
//int HM_Id = 37401;
...
//        cout << MOS_DataFileName[2*i+j](0,pos1) << endl;
        ...
        ofstream xml_file;
//        xml_file.open(Data_Dir+"/"+MOS_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
        xml_file.open("./XML_Info/"+xml_filename);
...
//        MOS_xml.SaveDoc(MOS_xmldoc, Data_Dir+"/"+MOS_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
        MOS_xml.SaveDoc(MOS_xmldoc, "./XML_Info/"+xml_filename);
...
{    //  AC_ampl(V)=0.250        AC_freq(Hz)=10.000E+3   Settling_time(s)=1000.0 , Temperature(C)=22.1
   //   gROOT->Reset();
   gStyle->SetOptStat(0);
   //TString Data_Dir = "/home/akyriakis/MOS-Measurements_Analysis/PQC_Measurements/Data/";
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<12; j++){
         ...
         cout << xml_filename << endl;
         ofstream xml_file;
         //        xml_file.open(Data_Dir+"/"+MOS_DataFileName[4*i+j](0,pos1)+"/"+xml_filename[4*i+j]);
         xml_file.open("./XML_Info/" +xml_filename);
         //xml_filename.open(Form("",HM_Id));
        // First create engine
        TXMLEngine VDP_xml;
        ...
        // Save document to file
        VDP_xml.SaveDoc(VDP_xmldoc, "./XML_Info/" +xml_filename);
        // Release memory before exit
...
void FET_PQC_xml_production(int xml_on, int nFiles){
 ...
 //        xml_file.open(Data_Dir+"/"+MOS_DataFileName[2*i+j](0,pos1)+"/"+xml_filename[2*i+j]);
        ...
        // Save document to file
        FET_xml.SaveDoc(FET_xmldoc, "./XML_Info/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    ...
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        cout << xml_filename << endl;
        ofstream xml_file;
        //        xml_file.open(Data_Dir+"/"+CAP_DataFileName[4*i+j](0,pos1)+"/"+xml_filename[4*i+j]);
        xml_file.open("./XML_Info/"+xml_filename);
        //xml_filename.open(Form("",HM_Id));
...
//              CAP_xml.NewChild(DATA, 0, "FILE_NAME", xml_filename[4*i+j]);
        ...
        // Save document to file
        //        CAP_xml.SaveDoc(CAP_xmldoc, Data_Dir+"/"+CAP_DataFileName[4*i+j](0,pos1)+"/"+xml_filename[4*i+j]);
        CAP_xml.SaveDoc(CAP_xmldoc, "./XML_Info/"+xml_filename);
        // Release memory before exit
...
void Flute1_2_S_Quick_Characterization_with_xml(int nFiles)
{
   ...
   CVS_Output_File.open(Form("CSV_Info/Flute1_VPX%d_0xx_2-S_HM_E_W_Left_Quick_Info.csv",HM_Id));
