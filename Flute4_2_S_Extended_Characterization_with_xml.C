  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<6; j++){
         ...
         xml_file.open("./XML_Info_Flute4/" +xml_filename);
        ...
        VDP_CCStructures_xml.SaveDoc(VDP_CCStructures_xmldoc, "./XML_Info_Flute4/" +xml_filename);
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        // Save document to file
        CBKRPoly_xml.SaveDoc(CBKRPoly_xmldoc, "./XML_Info_Flute4/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<4; j++){
        ...
        // Save document to file
        CBKRStrip_xml.SaveDoc(CBKRStrip_xmldoc, "./XML_Info_Flute4/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    ...
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        GCD05_xml.SaveDoc(GCD05_xmldoc, "./XML_Info_Flute4/" +xml_filename);
        // Release memory before exit
...
void Flute4_2_S_Extended_Characterization_with_xml(int nFiles)
{
   ...
   CVS_Output_File.open(Form("CSV_Info/Flute4_VPX%d_0xx_2-S_HM_E_W_Left_Extended_Info.csv",HM_Id));
