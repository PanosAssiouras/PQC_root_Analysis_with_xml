...
void Diel_IV_PQC_xml_production(int xml_on, int nFiles){
 ...
 //        cout << MOS_DataFileName[6*i+j](0,pos1) << endl;
         ...
         ofstream xml_file;
 //        xml_file.open(Data_Dir+"/"+MOS_DataFileName[6*i+j](0,pos1)+"/"+xml_filename[6*i+j]);
         xml_file.open("./XML_Info_Flute2/" +xml_filename);
        ...
        Diel_IV_xml.SaveDoc(Diel_IV_xmldoc, "./XML_Info_Flute2/" +xml_filename);
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        LineWidthStop_xml.SaveDoc(LineWidthStop_xmldoc, "./XML_Info_Flute2/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        LineWidthStrip_xml.SaveDoc(LineWidthStrip_xmldoc, "./XML_Info_Flute2/" +xml_filename);
        // Release memory before exit
...
double RPoly_Read_Info_from_File(TString RPoly_DataFileName, int id, int file_id)
{
   ...
   double R_sh = RPoly_A_slope[id]/1000000; // RPoly R_sh
  ...
  if(xml_on == 1){
    ...
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        RPoly_xml.SaveDoc(RPoly_xmldoc, "./XML_Info_Flute2/" +xml_filename);
        // Release memory before exit
  ...
  if(xml_on == 1){
    ...
    for(int i=0;i<nFiles;i++) {
      for(int j=0; j<=1; j++){
        ...
        // Save document to file
        GCD_xml.SaveDoc(GCD_xmldoc, "./XML_Info_Flute2/" +xml_filename);
        // Release memory before exit
...
void Flute2_2_S_Extended_Characterization_with_xml(int nFiles)
{
   ...
   CVS_Output_File.open(Form("CSV_Info/Flute2_VPX%d_0xx_2-S_HM_E_W_Left_Quick_Info.csv",HM_Id));
